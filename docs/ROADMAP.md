...
- **Docker note:** this build environment had **no Docker**, so every phase was verified via `pnpm check` (lint + typecheck + 268 unit tests, all green) — the live Docker-dependent paths (real fan-out, cascade-cancel, Bull-Board render, throttle 429/503, provider runs) are unit/typecheck-covered and flagged in each phase's status block for a Docker-lane pass.
