...
- **First action to pick tomorrow:** **P9b-core task 1 — per-provider BullMQ queues + routing** (the last P9b-core piece; a core-pipeline rewrite from the single `q.lookup` Nest `@Processor` to imperative per-provider `Worker`s, wants a worker/queue Testcontainers harness). ✅ **P8f-1**, ✅ **P9a**, ✅ **P9b-core wrappers** (`withBreaker` DB-persist + `withRateLimit` + `withSingleFlight`), and ✅ **P9b-core cancel-while-queued + cost counter** are **done and merged to `main`** (2026-07-02).
...
| Guardrails (P9) | 🟡 partial | **3 wrappers + cancel-while-queued + cost counter live.** `withBreaker` (Redis SM + DB-persist), `withRateLimit`, `withSingleFlight` in `apply-wrappers`; a persisted cancel flag stops queued jobs before they run; `cost:<provider>:<day>` INCR + `COST_DAILY_WARN`. Still open: per-provider BullMQ queues (P9b-core task 1); `@nestjs/throttler` + cost-cap enforcement = P9-pub. |
...
| 🟡 | **P9b-core** | **done + merged:** 3 wrappers, cancel-while-queued, cost counter. **Only remaining:** per-provider BullMQ queues (task 1 — imperative-Worker rewrite) | M | P9a | #2 |
...
> **Status (2026-07-02): all 3 wrappers done + merged.** `withBreaker` (Redis SM + DB-persist via `upsertHealth`), `withRateLimit` (per-provider fixed-window), and `withSingleFlight` (Redis `SET NX PX` lock + pub/sub fan-out) are live in `apply-wrappers` — order tracing → cache → single-flight → breaker → rate-limit → provider — each unit-tested with a fake Redis (injected clock / pub-sub bus). **Also done + merged (2026-07-02):** cancel-while-queued fix (task 4 — a persisted Redis flag checked at the top of `process()`, plus `job.remove()` + terminal-Cancelled emit for still-`waiting` jobs) and the cost counter (task 5 — `cost:<provider>:<YYYYMMDD>` INCR + `COST_DAILY_WARN`), extracted logic unit-tested in `apps/worker/src/lookups/guards.ts`. **Only remaining:** per-provider BullMQ queues + routing (task 1) — a core-pipeline rewrite from the single `q.lookup` Nest `@Processor` to imperative per-provider `Worker`s; land it with a worker/queue Testcontainers integration test (none exists yet — only `packages/db` uses Testcontainers).
