...
| Guardrails (P9) | 🟢 P9b-core done | **3 wrappers + cancel + cost + per-provider queues all live.** `withBreaker` (Redis SM + DB-persist), `withRateLimit`, `withSingleFlight` in `apply-wrappers`; persisted cancel flag; `cost:<provider>:<day>` INCR + `COST_DAILY_WARN`; **per-provider BullMQ queues + routing merged 2026-07-17.** Still open: `@nestjs/throttler` + cost-cap enforcement = P9-pub. |
