...
- **Where we are now (2026-07-17):** **every roadmap phase from P8f-1 through P9-pub is done and merged to `main`** — ✅ P8f-1, ✅ P9a, ✅ P9b-core, ✅ P13 (ops cockpit), ✅ P8f-2/Hudson Rock, ✅ P12 (search orchestration — the fan-out product layer), ✅ P14 (entitlement gate + payment stub), 🟡 P10 (Prometheus metrics + OTLP done; worker per-run spans follow-up), ✅ P9-pub (public hardening, env-gated default-off). **Only intentionally-out-of-scope work remains: [P11 deploy — DEFERRED by hard rule]** and **P15 real payments (needs P11)**. **First action when deploy is greenlit:** close the PII/retention TODO (§7.2), then activate P11.
- **Docker verification (done 2026-07-17):** brought the full stack up (`docker compose up -d --build`) and exercised the live flows end-to-end. ✅ **19 `@echo/db` integration tests** (Testcontainers, real Postgres) — all migrations apply, cascade FK works. ✅ A **username search fanned out to 5 real providers** (sherlock/maigret/whatsmyname/socialscan/hudsonrock) → **merged deduped report of 82 accounts**, all succeeded. ✅ Live SSE (lookup + search), cascade-cancel (running→cancelled), admin cockpit + Bull-Board auth (401/200), provider toggle→503, breaker-reset→closed, entitlement `paidAt`, `echo_*` Prometheus gauges. **One bug fixed from the live run:** `ignorant` was silently dropped from phone fan-out (needs a split `{country_code, phone}`) — made intentional + documented. `pnpm check` (268 unit) + `pnpm -F @echo/db test:int` (19) green. Remaining Docker-lane items (non-blocking): a fan-out peak-RSS load test (§3 numbers), and the 429/503 hardening responses (flip the P9-pub knobs on).
- **Spike/follow-up items (non-blocking):** P8f-2 **user-scanner** (undocumented CLI JSON export ⇒ Docker/Python integration spike); P10 **worker per-run counters + custom parent/child spans** (needs a worker metrics endpoint).
...
| Guardrails (P9) | 🟢 P9b-core + P9-pub done | **3 wrappers + cancel + cost + per-provider queues + public hardening all live.** `withBreaker` (Redis SM + DB-persist), `withRateLimit`, `withSingleFlight`; persisted cancel flag; `cost` counter + `COST_DAILY_WARN`; per-provider BullMQ queues; **P9-pub public hardening (per-IP throttle / backpressure / cost-cap / Turnstile) merged 2026-07-17, env-gated default-off.** |
| Concurrency | 🟢 P9b-core done | **P9a:** sidecar per-provider + global-heavy `asyncio.Semaphore` (default-deny) + per-container `mem_limit`/`cpus`. **P9b-core:** one BullMQ `Worker` per provider on `q.<id>`, concurrency = `provider.defaults.maxConcurrent` (mirror of the sidecar semaphore); the single `q.lookup`/`@Processor` is gone. Sidecar global-heavy semaphore remains the box-wide ceiling. |
...
| ✅ | **P9-pub** | Public hardening (per-IP throttle, backpressure, cost-cap enforce, Turnstile scaffold) — *done + merged 2026-07-17, all env-gated default-OFF; flip on just before P11* | M | P9b-core | #2 |
...
> **Status (2026-07-17): P9b-core COMPLETE + merged.** All 3 wrappers (`withBreaker` DB-persist, `withRateLimit`, `withSingleFlight`), cancel-while-queued, and the cost counter were done 2026-07-02. **Task 1 — per-provider BullMQ queues + routing — merged 2026-07-17:** the single `q.lookup` Nest `@Processor` is replaced by imperative BullMQ, fully out of `@nestjs/bullmq`. Producer side = `QueueRouter` (`apps/api/src/lookups/queue-router.ts`) building one `Queue` per provider (`q.<id>`) and routing enqueue + cancel-`getJob` by `providerId`. Consumer side = `LookupWorkers` (`apps/worker/src/lookups/lookup-workers.ts`) building one `Worker` per provider at `concurrency = provider.defaults.maxConcurrent` (the mirror of the P9a sidecar semaphore), each delegating to the shared provider-agnostic `LookupRunner` (extracted verbatim from the old processor). Unit-tested with a mocked `bullmq` (routing, per-queue concurrency sizing, delegation, shutdown) — 225 unit tests green. **Docker was absent in the build env, so the Testcontainers worker/queue integration test is deferred to a Docker-available lane** (unit coverage stands in). **Review follow-ups (non-blocking):** (a) derive the api+worker provider arrays from ONE shared list so they can't drift (per-provider queues turn a divergence into a silent `waiting`-forever hang — mitigated for now by a lockstep comment + `QueueRouter.has()` guard on cancel); (b) confirm target Redis `maxclients` before P11 — the worker now peaks ~90 connections (one Worker/provider + one cancel-subscriber per active job) vs ~3 on the old single queue.
...
> **Status (2026-07-17): DONE + merged, all DEFAULT-OFF.** `apps/api/src/hardening/` — one `PublicHardeningGuard` on **only** the two public POST routes (before `EntitlementGuard`), running four env-gated checks, cheapest first: **Turnstile** (`TURNSTILE_SECRET` set ⇒ verify the `cf-turnstile-response` token via Cloudflare siteverify → 403), **per-IP rate limit** (`RATE_LIMIT_PER_MINUTE`>0 ⇒ Redis fixed-window INCR → 429; a lightweight in-house limiter reusing the existing fixed-window pattern rather than pulling in `@nestjs/throttler`+a storage adapter), **backpressure** (`QUEUE_BACKPRESSURE_MAX`>0 ⇒ total waiting jobs over it → 503 `Retry-After`), **cost cap** (`COST_DAILY_CAP`>0 ⇒ today's total run-count over it → 503 `Retry-After`). Every knob defaults to 0/empty, so local dev + CI are never throttled; the pure decision helpers (`hardening.decisions.ts`, strict-over semantics) are unit-tested and the knobs surface in `/api/admin/config`. `pnpm check` green (268 tests). **Docker absent ⇒ the live 429/503/403 responses were not exercised** (unit-tested decisions + typecheck stand in); flip the knobs on and load-test just before P11. **Deviation from plan:** in-house Redis rate-limit instead of `@nestjs/throttler` (functionally equivalent to the DoD, fewer deps).
>
...
- ✅ **`P9-pub` — done, merged 2026-07-17.** Per-IP throttle + backpressure + cost-cap + Turnstile scaffold — all env-gated default-OFF; flip on just before P11.
- **All roadmap phases through P9-pub are now merged.** Remaining: **P11 deploy — DEFERRED** (hard rule; needs owner "activate P11" + the PII/retention TODO closed) and **P15 real payments — LAST** (needs P11). Plus two spike/follow-up items: P8f-2 user-scanner (Docker/Python lane) and P10 worker per-run spans/counters.
